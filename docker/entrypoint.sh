...
mkdir -p /usr/share/nginx/html/legacy/scripts/configs/ && envsubst < /usr/share/nginx/config.template.json > /usr/share/nginx/html/legacy/scripts/configs/config.json

cp /usr/share/nginx/html/legacy/index.orig.html /usr/share/nginx/html/legacy/index.html
[[ ! -z "${TITLE}" ]] && sed -i "s/<title>Waldur | Cloud Service Management<\/title>/<title>${TITLE}<\/title>/" /usr/share/nginx/html/legacy/index.html
