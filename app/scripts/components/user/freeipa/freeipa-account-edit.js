  ...
  controller: class FreeIPAAccountEditController {
    ...
    disableProfile() {
      ...
      this.freeipaService.disableProfile(this.profile.uuid).then(() => {
        ...
        this.ncUtilsFlash.success(gettext('Your FreeIPA profile has been disabled.'));
      }).finally(() => {
        this.loading = false;
      });
    }
    ...
    syncProfile() {
      this.loading = true;
      this.freeipaService.syncProfile(this.profile.uuid).then(response => {
        if (response.status === 204) {
          this.ncUtilsFlash.success(gettext('Your FreeIPA has been removed in FreeIPA server.'));
          return this.onProfileRemoved();
        } else {
          this.ncUtilsFlash.success(gettext('Your FreeIPA has been synced successfully.'));
        }
      }).finally(() => {
        this.loading = false;
      });
