...
vi.mock('@/navigation/useTabs', () => ({
  isDescendantOf: (parent: string, state: { name: string }) =>
    state.name.startsWith(`${parent}.`),
}));
...
vi.mock('@/ai-assistant/anonymous/AnonymousThreadProvider', () => ({
  useAnonymousThreadContext: () => ({
    ...
    isRunning: false,
    hasUnreadReply: anonymousHasUnreadReply,
    setHasUnreadReply: vi.fn(),
  }),
}));
...
describe('LLMChatDrawerToggle', () => {
  ...
  it('stays out of the way on the auth callbacks', () => {
    setState('home.logout_completed');
  ...
  it("blinks for a visitor's unread reply, held in shared state so any entry point can clear it", () => {
    vi.mocked(useUser).mockReturnValue(undefined);
    vi.mocked(isAnonymousVisitor).mockReturnValue(true);
    anonymousMessages = [{ role: 'assistant' }];
    anonymousHasUnreadReply = true;
