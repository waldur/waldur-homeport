...
export default function InvitationCancelAction(
  ...
  return context => ({
    ...
    callback: function(invitation) {
      invitationService
        .cancel(invitation.uuid)
        .then(function() {
          ncUtilsFlash.success(
            $filter('translate')(gettext('Invitation has been canceled.')),
          );
        ...
        .catch(function() {
          ncUtilsFlash.error(
            $filter('translate')(gettext('Unable to cancel invitation.')),
          );
        });
    },
    ...
    tooltip: function(invitation) {
      if (!InvitationPolicyService.canManageInvitation(context, invitation)) {
        return $filter('translate')(
          gettext("You don't have permission to cancel this invitation."),
        );
      }
      if (invitation.state !== 'pending') {
        return $filter('translate')(
          gettext('Only pending invitation can be canceled.'),
        );
      }
    },
  });
