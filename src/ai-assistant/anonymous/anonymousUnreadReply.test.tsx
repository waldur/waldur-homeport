...
import { AskAboutOfferingButton } from '@/marketplace/offerings/AskAboutOfferingButton';
...
// drawer the visitor had opened from an offering page.
...
const OFFERING = { name: 'Elastic HPC', customer_name: 'CSC' } as any;
...
const Page: FC<{ running: boolean }> = ({ running }) => (
  <>
    ...
    <LLMChatDrawerToggle />
    <AskAboutOfferingButton offering={OFFERING} />
  </>
);
...
describe('visitor unread reply marker', () => {
  ...
  it('clears when the drawer is opened from an offering, not just from the header', async () => {
    const { rerender } = render(
      <Wrapper>
        <Page running={false} />
      </Wrapper>,
