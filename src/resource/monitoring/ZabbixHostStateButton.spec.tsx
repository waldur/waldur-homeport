...
describe('ZabbixHostStateButton', () => {
  it('renders valid message if resource is OK', () => {
    ...
    expect(wrapper.find(ZabbixHostDetailsButton).prop('textClass')).toBe(
      'text-info',
    );
    expect(wrapper.find(ZabbixHostDetailsButton).prop('label')).toBe(
      'Resource is monitored using Zabbix.',
    );
  });
