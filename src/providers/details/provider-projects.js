  ...
  angular.extend($scope, {
    ...
    getContext: function() {
      ...
      return currentStateService.getCustomer().then(customer => {
        ...
        return $q
          .all([
            this.getProjects(customer).then(
        ...
        .map(function(choice) {
          ...
          return joinServiceProjectLinkService
            ...
            .catch(function(response) {
              ...
              choice.subtitle =
                gettext('Unable to create link.') + ' ' + reason;
              choice.selected = false;
            });
        });

      let delete_promises = this.choices
        .filter(function(choice) {
          return !choice.selected && choice.link_url;
        })
        .map(function(choice) {
          ...
          return joinServiceProjectLinkService
            ...
            .catch(function(response) {
              ...
              choice.subtitle =
                gettext('Unable to delete link.') + ' ' + reason;
              choice.selected = true;
            });
        });

      return $q
        .all(add_promises.concat(delete_promises))
        .then(() => {
          $scope.$broadcast('onSave');
        })
        .then(() => {
          joinServiceProjectLinkService.clearAllCacheForCurrentEndpoint();
        });
    },
  });
  $scope.init();
