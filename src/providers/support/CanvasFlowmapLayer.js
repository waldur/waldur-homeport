...
(function(factory, window) {
  ...
  if (typeof window !== 'undefined' && window.L) {
    ...
    window.L.canvasFlowmapLayer = function(
      originAndDestinationGeoJsonPoints,
      opts,
    ) {
      return new window.L.CanvasFlowmapLayer(
        originAndDestinationGeoJsonPoints,
        opts,
      );
    };
  ...
  let CanvasFlowmapLayer = L.GeoJSON.extend({
    ...
    initialize: function(geoJson, options) {
      ...
      if (TWEEN) {
        ...
        this._animationTween = new TWEEN.Tween(this._animationPropertiesStatic)
          ...
          .easing(
            this._animationPropertiesDynamic.easingInfo.tweenEasingFunction,
          )
    ...
    setOriginAndDestinationGeoJsonPoints: function(geoJsonFeatureCollection) {
      if (geoJsonFeatureCollection.features) {
        var configOriginGeometryObject = this.options
          .originAndDestinationFieldIds.originGeometry;
        var configDestinationGeometryObject = this.options
        ...
        var geoJsonPointsToDraw = this._filterGeoJsonPointsToDraw(
    ...
    selectFeaturesForPathDisplayById: function(
      ...
      var existingOriginOrDestinationFeature = this.originAndDestinationGeoJsonPoints.features.filter(
        function(feature) {
          return (
      ...
      var odInfo = this._getSharedOriginOrDestinationFeatures(
    ...
    _filterGeoJsonPointsToDraw: function(geoJsonFeatureCollection) {
      ...
      var originUniqueIdField = this.options.originAndDestinationFieldIds
        .originUniqueIdField;
      var destinationUniqueIdField = this.options.originAndDestinationFieldIds
    ...
    _drawSelectedCanvasPaths: function(animate) {
      ...
      var originAndDestinationFieldIds = this.options
        .originAndDestinationFieldIds;
      ...
      this.originAndDestinationGeoJsonPoints.features.forEach(function(
        ...
        if (feature.properties._isSelectedForPathDisplay) {
          ...
          var originLatLng = this._wrapAroundLatLng(
            L.latLng([originYCoordinate, originXCoordinate]),
          );
          var destinationLatLng = this._wrapAroundLatLng(
            L.latLng([destinationYCoordinate, destinationXCoordinate]),
          );

          // convert geometry to screen coordinates for canvas drawing
          var screenOriginPoint = this._map.latLngToContainerPoint(
            originLatLng,
          );
          var screenDestinationPoint = this._map.latLngToContainerPoint(
            destinationLatLng,
          );
          ...
          if (animate) {
            ...
            ctx.beginPath();
            this._applyAnimatedCanvasLineSymbol(
              ctx,
              symbol,
              screenOriginPoint,
              screenDestinationPoint,
    ...
    _getSymbolProperties: function(feature, canvasSymbolConfig) {
      ...
      if (canvasSymbolConfig.type === 'simple') {
        ...
        filteredSymbols = canvasSymbolConfig.classBreakInfos.filter(function(
          ...
          return (
            info.classMinValue <=
              feature.properties[canvasSymbolConfig.field] &&
    ...
    _applyAnimatedCanvasLineSymbol: function(
      ctx,
      symbolObject,
      screenOriginPoint,
      screenDestinationPoint,
