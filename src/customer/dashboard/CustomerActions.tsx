...
export const CustomerActions = () => {
  ...
  return (
    <div>
      <Link state="organization.manage" className="btn btn-secondary me-3">
        <SvgIcon path={IconCog} className="svg-icon-2" />
        {translate('Manage')}
      </Link>

      <Link state="organization.events" className="btn btn-secondary me-3">
        <SvgIcon path={IconLogs} className="svg-icon-2" />
        {translate('Audit logs')}
      </Link>

      {showIssues && (
        <Link state="organization.issues" className="btn btn-secondary">
