...
import { getFormLimitSerializer, getFormLimitParser, Limits } from '@waldur/marketplace/common/registry';
...
const transformLimits = (func, limits: OfferingLimits): OfferingLimits => {
  const minValues = func(Object.keys(limits).reduce(
    (rv, key) => ({
      ...rv,
      [key]: limits[key].min,
    }), {}));

  const maxValues = func(Object.keys(limits).reduce(
    (rv, key) => ({
      ...rv,
      [key]: limits[key].max,
    }), {}));
...
export const serializeLimitValues = (type: string, limits: OfferingLimits): OfferingLimits => {
  const limitSerializer = getFormLimitSerializer(type);
  return transformLimits(limitSerializer, limits);
};

const parseLimitValues = (type: string, limits: OfferingLimits): OfferingLimits => {
...
export const getDefaults = (offering: Offering) => {
  ...
  return Object.keys(limits).reduce((rv, key) => ({
    ...rv,
    [key]: limits[key].min,
  }), {});
};
