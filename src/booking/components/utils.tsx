...
export const bookingDataTemplate = event => {
  ...
  const getLabelValues = getLabels.map(label => ({label, value: event[label]}));
...
export const renderEventWithTooltip =
  ({event, el}: {event: EventApi, el: HTMLElement}) => ReactDOM.render(
  <OverlayTrigger
    placement="top"
    overlay={<BootstrapTooltip id={event.id}>
    ...
    <div className={el.children[0].className}>
      {
        event.title
          ...
          : <span className="fc-time">{formatTime(event.end)} - {formatTime(event.start)}</span>
      }
    </div>
  </OverlayTrigger>,
  el
...
export const eventRender = (info: {event: EventApi, el: HTMLElement, withTooltip?: boolean }) => {
  const {event, el, withTooltip} = info;
  if (el.classList.contains('progress')) {
    el.children[0].className = classNames( el.children[0].className, 'progress-bar', 'full-width ', {
