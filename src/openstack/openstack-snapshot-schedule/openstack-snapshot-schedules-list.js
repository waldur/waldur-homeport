...
function openstackSnapshotSchedulesListController(
  ...
  let controllerClass = baseResourceListController.extend({
    init: function() {
      ...
      this.loading = true;
      actionUtilsService
        .loadNestedActions(this, controllerScope.resource, 'snapshot_schedules')
        .then(result => {
    ...
    getTableOptions: function() {
      ...
      options.noMatchesText = gettext(
        'No snapshot schedules found matching filter.',
      );
      ...
      options.columns = [
        ...
        },
        {
          ...
          render: row =>
            `<i class="fa ${row.is_active ? 'fa-check' : 'fa-minus'}"></i>`,
        },
        {
          title: gettext('State'),
          className: 'min-tablet-l',
