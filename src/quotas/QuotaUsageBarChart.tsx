...
export const exceeds = quota =>
  quota.usage + quota.required > quota.limit;
      ...
      if (quota.limit !== -1) {
        return  (
          <React.Fragment key={index}>
            ...
            <Tooltip id="quota-usage" label={<ProgressTooltipMessage quota={quota}/>}>
              <ProgressBar>
                <ProgressBar bsStyle="success" now={quota.usage * 100 / quota.limit} key={1}/>
                <ProgressBar bsStyle="warning" now={quota.required * 100 / quota.limit} key={2}/>
              </ProgressBar>
            </Tooltip>
          </React.Fragment>
        );
