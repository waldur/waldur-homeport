...
class GreekTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const greekContent = fs.readFileSync(
        path.join(rootDir, 'locales/el.json'),
        'utf8',
      );
      ...
      console.log(
        `🇬🇷 Loaded ${Object.keys(this.greekTranslations).length} Greek translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, greek, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateGreekVerb(english, greek)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Greek imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasInconsistentGreekCase(greek)) {
      issues.push({
        ...
        message:
          'Check Greek case usage (4 cases) - ensure consistency with context',
        severity: 'high',
      });
    }
    ...
    if (this.hasIncorrectDefiniteArticle(greek)) {
      issues.push({
        ...
        message:
          'Check Greek definite article agreement (ο, η, το and their forms)',
        severity: 'medium',
      });
    }
    ...
    if (this.hasMissingAccents(greek)) {
      issues.push({
        type: 'accent_marks',
        message:
          'Ensure proper Greek accent marks (οξεία ´) in monotonic system',
        severity: 'medium',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkGreekVariableHandling(
          english,
          greek,
          context.variables,
        );
  ...
  checkGreekVariableHandling(english, greek, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperGreekNumberAgreement(greek, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'Greek number-noun agreement needs attention (singular/plural forms)',
            severity: 'high',
          });
        }
      }
      ...
      if (
        varInfo.type === 'string' &&
        this.needsGreekCaseAdjustment(greek, varName)
      ) {
        issues.push({
  ...
  checkGreekTitle(english, greek) {
    ...
    if (this.needsArticleInTitle(greek)) {
      issues.push({
        ...
        message:
          'Consider if Greek definite article is needed in title context',
        severity: 'low',
      });
    }
  ...
  hasAppropriateGreekVerb(english, greek) {
    ...
    const greekImperativePatterns = [
      /[εη]$/,
      /[ετε]ετε$/,
      /[στε]στε$/, // Common imperative endings
    ];
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      return greekImperativePatterns.some((pattern) =>
        pattern.test(greek.toLowerCase()),
      );
    }
  ...
  hasInconsistentGreekGender(greek) {
    ...
    return (
      greek.match(/\sο\s.*[αη]$/) || // masculine article with feminine ending
      greek.match(/\sη\s.*[οςμ]$/) || // feminine article with masculine ending
      greek.match(/\sτο\s.*[αεηου]$/)
    ); // neuter article with non-neuter ending
  }
  ...
  hasInconsistentFormality(greek) {
    ...
    return (
      (greek.includes('εσείς') && greek.includes('εσύ')) ||
  ...
  hasMissingAccents(greek) {
    ...
    return wordsNeedingAccents.some(
      (word) =>
        greek.toLowerCase().includes(word) &&
        !greek.includes(
          word.replace('ο', 'ό').replace('η', 'ή').replace('α', 'ά'),
        ),
    );
  }
  ...
  hasImproperGreekAlphabet(greek) {
    ...
    return (
      /[a-zA-Z]/.test(greek) || // Latin characters
      (greek.includes('ς') && !greek.match(/ς\s|ς$|ς\.|ς,|ς;/))
    ); // Final sigma in wrong position
  }
  ...
  hasProperGreekNumberAgreement(greek, varName) {
    ...
    if (greek.includes(numberVar)) {
      ...
      return (
        greek.includes('|') || // Indicates plural handling
        greek.match(/\{.*\}\s+[α-ωά-ώ]+[ες|α|ων]$/)
      ); // Plural endings
    }
    return true;
  }
  ...
  needsVerbConjugationConsistency(greek, varName) {
    ...
    return (
      greek.includes(`{${varName}}`) &&
      (greek.includes('θα') ||
        greek.includes('έχει') ||
        greek.includes('είναι'))
    );
  }

  needsDefiniteArticleAgreement(greek, varName) {
    // Check if variable needs definite article agreement
    return greek.includes(`{${varName}}`) && greek.match(/\s(ο|η|το|οι|τα)\s/);
  }
  ...
  isGreekErrorTooAbrupt(greek) {
    ...
    return abruptPatterns.some((pattern) => {
      return (
        greek.toLowerCase().startsWith(pattern) && !greek.includes('παρακαλώ')
      );
    });
  }
  ...
  needsArticleInTitle(greek) {
    ...
    return (
      greek.split(' ').length > 1 &&
      !greek.match(/^(ο|η|το|οι|τα)\s/) &&
      !greek.includes('νέ')
    ); // Not starting with article and not "new"
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log(
      '1. Focus on case system implementation (4 cases) and article agreement',
    );
