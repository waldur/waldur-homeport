...
class RussianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const russianContent = fs.readFileSync(path.join(rootDir, 'locales/ru.json'), 'utf8');
      ...
      console.log(`🇷🇺 Loaded ${Object.keys(this.russianTranslations).length} Russian translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, russian, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateRussianVerb(english, russian)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using imperative verb form for action buttons in Russian',
          severity: 'low'
        });
      }
    }
  ...
  isAppropriateDeleteTranslation(russian) {
    const strongDeleteTerms = ['удалить', 'удали', 'удаление', 'стереть', 'уничтожить'];
  ...
  analyzeVariableTranslations() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      const context = templateData.context;
      if (context && context.variables && Object.keys(context.variables).length > 0) {
        const issues = this.checkVariableHandling(english, russian, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            russian,
  ...
  checkVariableHandling(english, russian, variables) {
    ...
    const numberVars = Object.entries(variables).filter(([_, data]) => data.type === 'number');
    if (numberVars.length > 0) {
      const wordOrderIssue = this.checkRussianNumberWordOrder(english, russian, numberVars);
      if (wordOrderIssue) {
  ...
  checkRussianNumberWordOrder(english, russian, numberVars) {
    ...
    for (const [varName] of numberVars) {
      ...
      const textAfterVar = russian.substring(varIndex + varPattern.length).trim();
  ...
  checkTitleQuality(english, russian) {
    ...
    if (this.hasInappropriateRussianCapitalization(english, russian)) {
      issues.push({
        type: 'capitalization',
        message: 'Russian title capitalization differs from English - check if current style is appropriate',
        severity: 'low'
      });
    }
    ...
    if (russian.length > english.length * 3) {
      issues.push({
        type: 'title_length',
        message: 'Title translation significantly longer - consider more concise Russian equivalent',
        severity: 'medium'
      });
    }
  ...
  hasInappropriateRussianCapitalization(english, russian) {
    ...
    if (englishWordCount > 1 && russianWordCount > 1) {
      const englishCapitalWords = english.split(/\s+/).filter(word => /^[A-Z]/.test(word)).length;
      const russianCapitalWords = russian.split(/\s+/).filter(word => /^[А-ЯЁ]/.test(word)).length;
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    if (buttonIssues.length > 0) {
      ...
      if (buttonIssues.length > 10) {
        console.log(`   ... and ${buttonIssues.length - 10} more button issues\n`);
      }
    }
    ...
    if (variableIssues.length > 0) {
      ...
      variableIssues.slice(0, 10).forEach((issue, index) => {
        console.log(`${index + 1}. "${issue.english}" → "${issue.russian}"`);
        console.log(`   Variables: {${Object.keys(issue.variables).join('}, {')}}`);
        issue.issues.forEach(i => console.log(`   ⚠️  ${i.message}`));
        console.log('');
      });
      if (variableIssues.length > 10) {
        console.log(`   ... and ${variableIssues.length - 10} more variable issues\n`);
      }
    }
    ...
    this.generateSummaryRecommendations(buttonIssues, variableIssues, titleIssues);
  }
  ...
  generateSummaryRecommendations(buttonIssues, variableIssues, titleIssues) {
    ...
    if (buttonIssues.length > 0) {
      recommendations.push('• Review button text for appropriate Russian imperative verb forms');
      recommendations.push('• Consider shorter, more concise button labels');
      recommendations.push('• Use stronger terminology for destructive actions (delete buttons)');
      recommendations.push('• Watch for common verb form errors: "reload" = "перезагрузить" (NOT "перезаряжать")');
      recommendations.push('• Watch for common noun errors: "staff" = "персонал" (NOT "посох" which means walking stick)');
      recommendations.push('• Distinguish action verbs: load/download vs charge, refresh vs reload');
    }
    
    if (variableIssues.length > 0) {
      recommendations.push('• Implement Russian number-noun agreement for count variables');
      recommendations.push('• Review variable positioning for natural Russian word order');
      recommendations.push('• Add conditional plural forms where needed');
    }
    
    if (titleIssues.length > 0) {
      recommendations.push('• Use Russian sentence case instead of English title case');
      recommendations.push('• Optimize title length for Russian language patterns');
    }
    
    if (recommendations.length === 0) {
      console.log('✅ Russian translations appear to be well-adapted to the context!');
    } else {
      recommendations.forEach(rec => console.log(rec));
    }
    
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log('1. Focus on high-severity issues first (missing variables, inappropriate delete terms)');
    console.log('2. Review button and action text for Russian language conventions');
    console.log('3. Implement proper number-noun agreement for dynamic content');
  }
}
