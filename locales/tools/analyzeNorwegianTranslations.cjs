...
class NorwegianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const norwegianContent = fs.readFileSync(
        path.join(rootDir, 'locales/nb.json'),
        'utf8',
      );
      ...
      console.log(
        `🇳🇴 Loaded ${Object.keys(this.norwegianTranslations).length} Norwegian translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, norwegian, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateNorwegianVerb(english, norwegian)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Norwegian imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.shouldUseNorwegianCompound(english, norwegian)) {
      issues.push({
        type: 'compound_word',
        message:
          'Consider Norwegian compound word construction for better readability',
        severity: 'low',
      });
    }
    ...
    if (this.hasUnnecessaryEnglishLoanwords(norwegian)) {
      issues.push({
        type: 'loanword_usage',
        message:
          'Consider using native Norwegian terms instead of English loanwords',
        severity: 'medium',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkNorwegianVariableHandling(
          english,
          norwegian,
          context.variables,
        );
  ...
  checkNorwegianVariableHandling(english, norwegian, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperNorwegianNumberAgreement(norwegian, varName)) {
          issues.push({
            ...
            message:
              'Norwegian number-noun agreement may need attention (singular/plural forms)',
            severity: 'high',
          });
        }
      }
      ...
      if (
        varInfo.type === 'string' &&
        this.needsNorwegianGenderAgreement(norwegian, varName)
      ) {
        issues.push({
  ...
  checkNorwegianTitle(english, norwegian) {
    ...
    if (this.shouldUseNativeNorwegianTerms(english, norwegian)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Norwegian terminology instead of loanwords',
        severity: 'medium',
      });
    }
  ...
  shouldUseNorwegianCompound(english, norwegian) {
    ...
    return (
      ...
      norwegianWords.length >= englishWords.length &&
      !norwegian.includes(' og ') &&
      !norwegian.includes(' eller ')
  ...
  hasUnnecessaryEnglishLoanwords(norwegian) {
    ...
    return commonLoanwords.some((loanword) =>
      norwegian.toLowerCase().includes(loanword),
    );
  }
  ...
  shouldUseNativeNorwegianTerms(english, norwegian) {
    ...
    return loanwordPatterns.some((pattern) =>
      norwegian.toLowerCase().includes(pattern),
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log(
      '1. Focus on variable handling and gender agreement issues first',
    );
    ...
    console.log(
      '5. Replace English loanwords with native Norwegian terms where appropriate',
    );
    console.log(
      '6. Consider Norwegian compound word construction for technical terms',
    );
  }
}
