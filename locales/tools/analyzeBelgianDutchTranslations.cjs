...
class BelgianDutchTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const belgianDutchContent = fs.readFileSync(
        path.join(rootDir, 'locales/nl-BE.json'),
        'utf8',
      );
      ...
      console.log(
        `🇧🇪 Loaded ${Object.keys(this.belgianDutchTranslations).length} Belgian Dutch translations`,
      );
    } catch (error) {
    ...
    if (
      ...
      if (!this.hasAppropriateBelgianDutchVerb(english, belgianDutch)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Belgian Dutch imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkBelgianDutchVariableHandling(
          english,
          belgianDutch,
          context.variables,
        );
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      ...
      if (this.violatesBelgianDutchV2Rule(belgianDutch, varName)) {
        issues.push({
          ...
          variable: varName,
          message:
            'Variable placement may violate Belgian Dutch V2 rule variations',
          severity: 'medium',
        });
      }
    }
  ...
  checkBelgianDutchTitle(english, belgianDutch) {
    ...
    if (this.hasInappropriateBelgianDutchCapitalization(belgianDutch)) {
      issues.push({
        type: 'capitalization',
        message:
          'Belgian Dutch titles typically use sentence case, not title case',
        severity: 'low',
      });
    }
  ...
  hasAppropriateBelgianDutchVerb(english, belgianDutch) {
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      return (
        belgianDutchImperativePatterns.some((pattern) =>
          pattern.test(belgianDutch.toLowerCase()),
        ) ||
        belgianDutch
          .toLowerCase()
  ...
  lacksBelgianFormalAddress(belgianDutch) {
    ...
    return (
      informalMarkers.test(belgianDutch) && !formalMarkers.test(belgianDutch)
    );
  }
  ...
  shouldUseFrenchLoanwords(english, belgianDutch) {
    ...
    return Object.keys(frenchLoanwordOpportunities).some(
      (term) =>
  ...
  shouldUseNativeBelgianDutchTerms(english, belgianDutch) {
    ...
    return standardDutchTerms.some((term) =>
      belgianDutch.toLowerCase().includes(term),
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log(
      '4. Standardize title capitalization to Belgian Dutch conventions',
    );
