...
class PolishTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const polishContent = fs.readFileSync(path.join(rootDir, 'locales/pl.json'), 'utf8');
      ...
      console.log(`🇵🇱 Loaded ${Object.keys(this.polishTranslations).length} Polish translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, polish, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriatePolishVerb(english, polish)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Polish imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInconsistentPolishFormality(polish)) {
      issues.push({
        ...
        message: 'Check Pan/Pani/ty consistency - use formal address in professional contexts',
        severity: 'high'
      });
    }
    ...
    if (!this.hasProperPolishCaseUsage(polish)) {
      issues.push({
        type: 'case_usage',
        message: 'Check Polish case usage (nominative, genitive, dative, accusative, instrumental, locative, vocative)',
        severity: 'high'
      });
    }
    ...
    if (this.needsPolishAspectCheck(english, polish)) {
      issues.push({
        type: 'aspect_usage',
        message: 'Consider Polish verb aspect (perfective/imperfective) appropriateness',
        severity: 'medium'
      });
    }
    ...
    if (this.hasMissingPolishDiacritics(polish)) {
      issues.push({
        ...
        message: 'Check for missing diacritical marks (ą, ć, ę, ł, ń, ó, ś, ź, ż)',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkPolishVariableHandling(english, polish, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            polish,
  ...
  checkPolishVariableHandling(english, polish, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperPolishNumberAgreement(polish, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'Polish number-noun agreement needs attention (1, 2-4, 5+ patterns)',
            severity: 'high'
          });
        }
      }
      ...
      if (varInfo.type === 'string' && this.needsPolishCaseInflection(polish, varName)) {
        issues.push({
      ...
      if (this.needsPolishGenderAgreement(polish, varName)) {
        issues.push({
          type: 'gender_agreement',
          variable: varName,
          message: 'Variable may need Polish gender agreement (masculine/feminine/neuter)',
          severity: 'high'
        });
      }
    }
  ...
  checkPolishTitle(english, polish) {
    ...
    if (this.shouldUseNativePolishTerms(english, polish)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native Polish terminology instead of foreign borrowings',
        severity: 'medium'
      });
    }
  ...
  hasAppropriatePolishVerb(english, polish) {
    ...
    const polishImperativePatterns = [
      /ać$/, /eć$/, /ić$/, /ować$/,  // Infinitive endings
      /aj$/, /ij$/, /uj$/,          // Imperative endings
    ];
    ...
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      ...
      const commonActionVerbs = ['dodaj', 'zapisz', 'usuń', 'utwórz', 'aktualizuj', 'wyślij', 'anuluj'];
      return commonActionVerbs.some(verb => polish.toLowerCase().includes(verb)) ||
             polishImperativePatterns.some(pattern => pattern.test(polish.toLowerCase()));
    }
  ...
  hasInconsistentPolishFormality(polish) {
    ...
    const hasFormal = formalPatterns.some(pattern => polish.includes(pattern));
    const hasInformal = informalPatterns.some(pattern => polish.includes(pattern));
  ...
  hasProperPolishGenderAgreement(polish) {
    ...
    return !mismatchPatterns.some(pattern => pattern.test(polish.toLowerCase()));
  }
  ...
  needsPolishAspectCheck(english, polish) {
    ...
    const aspectSensitiveActions = ['save', 'create', 'update', 'delete', 'send'];
    
    if (aspectSensitiveActions.some(action => english.toLowerCase().includes(action))) {
      // Check if Polish uses appropriate perfective/imperfective forms
      const perfectiveMarkers = ['zapisać', 'utworzyć', 'zaktualizować', 'usunąć', 'wysłać'];
      const imperfectiveMarkers = ['zapisywać', 'tworzyć', 'aktualizować', 'usuwać', 'wysyłać'];
      
      return !perfectiveMarkers.some(marker => polish.includes(marker)) &&
  ...
  hasImproperConsonantClusters(polish) {
    ...
    return improperClusters.some(pattern => pattern.test(polish.toLowerCase()));
  }
  ...
  hasProperPolishNumberAgreement(polish, varName) {
    ...
    if (polish.includes(numberVar)) {
      ...
      return singularMarkers.some(marker => polish.includes(marker)) ||
  ...
  needsPolishCaseInflection(polish, varName) {
    ...
    const caseMarkers = ['do', 'od', 'z', 'w', 'na', 'o', 'przez', 'bez', 'dla'];
    return polish.includes(numberVar) && 
  ...
  isPolishErrorTooAbrupt(polish) {
    ...
    const abruptPatterns = ['błąd', 'niemożliwe', 'zabronione', 'nieprawidłowy'];
  ...
  needsPolishConditional(polish) {
    ...
    return !conditionalMarkers.some(marker => polish.includes(marker)) &&
  ...
  shouldUseNativePolishTerms(english, polish) {
    ...
    const foreignTerms = ['email', 'software', 'hardware', 'login', 'website', 'download'];
    
    return foreignTerms.some(foreign => polish.toLowerCase().includes(foreign));
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log('1. Ensure consistent use of formal address (Pan/Pani) throughout');
