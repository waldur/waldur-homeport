...
class FrenchTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const frenchContent = fs.readFileSync(path.join(rootDir, 'locales/fr.json'), 'utf8');
      ...
      console.log(`🇫🇷 Loaded ${Object.keys(this.frenchTranslations).length} French translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, french, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateFrenchVerb(english, french)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using French imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInconsistentFrenchFormality(french)) {
      issues.push({
        ...
        message: 'Check vous/tu consistency - use formal "vous" in professional contexts',
        severity: 'high'
      });
    }
    ...
    if (this.hasMissingFrenchAccents(french)) {
      issues.push({
        type: 'accent_marks',
        message: 'Check for missing or incorrect accent marks (é, è, ê, à, ç, etc.)',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkFrenchVariableHandling(english, french, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            french,
  ...
  checkFrenchVariableHandling(english, french, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperFrenchNumberAgreement(french, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'French number-noun agreement may need attention (singular/plural)',
            severity: 'high'
          });
        }
      }
      ...
      if (varInfo.type === 'string' && this.needsFrenchGenderAgreement(french, varName)) {
        issues.push({
          type: 'gender_agreement',
          variable: varName,
          message: 'Variable may need French gender agreement (masculine/feminine)',
          severity: 'high'
        });
      }
  ...
  checkFrenchTitle(english, french) {
    ...
    if (this.shouldUseNativeFrenchTerms(english, french)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native French terminology instead of Anglicisms',
        severity: 'medium'
      });
    }
  ...
  hasAppropriateFrenchVerb(english, french) {
    ...
    const frenchImperativePatterns = [
      /er$/, /ir$/, /re$/,     // Infinitive endings
      /ez$/, /ons$/,          // Imperative endings
    ];
    ...
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      ...
      const commonActionVerbs = ['ajouter', 'sauvegarder', 'supprimer', 'créer', 'mettre à jour', 'envoyer', 'annuler'];
      return commonActionVerbs.some(verb => french.toLowerCase().includes(verb)) ||
             frenchImperativePatterns.some(pattern => pattern.test(french.toLowerCase()));
    }
  ...
  hasInconsistentFrenchFormality(french) {
    ...
    const hasFormal = formalPatterns.some(pattern => french.toLowerCase().includes(pattern));
    const hasInformal = informalPatterns.some(pattern => french.toLowerCase().includes(pattern));
  ...
  hasProperFrenchGenderAgreement(french) {
    ...
    return !mismatchPatterns.some(pattern => pattern.test(french.toLowerCase()));
  }
  ...
  hasProperFrenchNumberAgreement(french, varName) {
    ...
    if (french.includes(numberVar)) {
      ...
      return pluralContext.match(/\{[^}]+\}\s+\w+(s|x)\b/) !== null ||
             french.includes('un') || french.includes('une'); // Singular forms
    }
    return true;
  }
  ...
  needsFrenchGenderAgreement(french, varName) {
    ...
    return french.includes(numberVar) && 
  ...
  needsFrenchLiaison(french, varName) {
    ...
    const numberVar = `{${varName}}`;
    if (french.includes(numberVar)) {
      const context = french.substring(french.indexOf(numberVar) - 10, french.indexOf(numberVar) + 20);
  ...
  hasAppropriateFrenchFormality(french) {
    ...
    return formalMarkers.some(marker => french.toLowerCase().includes(marker));
  }
  ...
  hasProperFrenchNegation(french) {
    ...
    if (french.includes('ne') || french.includes('n\'')) {
      ...
      const negationParts = ['pas', 'plus', 'jamais', 'rien', 'personne', 'aucun'];
  ...
  shouldUseNativeFrenchTerms(english, french) {
    ...
    return anglicisms.some(anglicism => french.toLowerCase().includes(anglicism));
  }
