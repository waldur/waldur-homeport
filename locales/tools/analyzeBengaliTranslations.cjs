...
class BengaliTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const bengaliContent = fs.readFileSync(
        path.join(rootDir, 'locales/bn.json'),
        'utf8',
      );
      ...
      console.log(
        `🇧🇩 Loaded ${Object.keys(this.bengaliTranslations).length} Bengali translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, bengali, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateBengaliVerb(english, bengali)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using appropriate Bengali verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasInconsistentBengaliAddress(bengali)) {
      issues.push({
        ...
        message:
          'Check formal/informal/intimate address consistency (আপনি/তুমি/তুই)',
        severity: 'high',
      });
    }
    ...
    if (this.needsHonorificUsage(bengali)) {
      issues.push({
        ...
        message:
          'Consider appropriate Bengali honorific usage for respectful tone',
        severity: 'medium',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkBengaliVariableHandling(
          english,
          bengali,
          context.variables,
        );
  ...
  checkBengaliTitle(english, bengali) {
    ...
    if (this.shouldUseNativeBengaliTerms(english, bengali)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Bengali terminology instead of loanwords',
        severity: 'medium',
      });
    }
    ...
    if (this.needsFormalBengaliLanguage(bengali)) {
      issues.push({
        type: 'formality',
        message:
          'Consider using formal Bengali language appropriate for titles',
        severity: 'low',
      });
    }
  ...
  hasAppropriateBengaliVerb(english, bengali) {
    ...
    const bengaliActionVerbs =
      /(যোগ করুন|সংরক্ষণ করুন|মুছে ফেলুন|তৈরি করুন|আপডেট করুন|পাঠান|বাতিল করুন)/;
