...
class ArabicTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const arabicContent = fs.readFileSync(
        path.join(rootDir, 'locales/ar.json'),
        'utf8',
      );
      ...
      console.log(
        `🇸🇦 Loaded ${Object.keys(this.arabicTranslations).length} Arabic translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, arabic, context) {
    ...
    if (!this.hasProperRTLHandling(arabic)) {
      issues.push({
        type: 'rtl_direction',
        message:
          'Ensure proper right-to-left text direction handling for Arabic',
        severity: 'high',
      });
    }
    ...
    if (
      ...
      if (!this.hasAppropriateArabicVerb(english, arabic)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Arabic imperative or command form for action buttons',
          severity: 'low',
        });
      }
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkArabicVariableHandling(
          english,
          arabic,
          context.variables,
        );
      ...
      if (varInfo.type === 'number') {
        if (!this.hasProperArabicNumberAgreement(arabic, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'Arabic number-noun agreement may need attention (1, 2, 3-10, 11+ forms)',
            severity: 'high',
          });
        }
      }
  ...
  checkArabicTitle(english, arabic) {
    ...
    if (this.shouldUseNativeArabicTerms(english, arabic)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Arabic terminology instead of transliterated loanwords',
        severity: 'medium',
      });
    }
  ...
  needsArabicGenderAgreement(arabic, varName) {
    ...
    return arabic.includes(variablePattern) && /[ء-ي]/.test(arabic); // Any Arabic text - simplified check
  }

  needsArabicDualForm(arabic, varName) {
    const variablePattern = `{${varName}}`;
    // Check if variable might represent a quantity of 2 (dual form needed)
    return arabic.includes(variablePattern) && arabic.includes('2'); // Simplified check
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log(
      '1. Focus on right-to-left (RTL) text direction handling first',
    );
