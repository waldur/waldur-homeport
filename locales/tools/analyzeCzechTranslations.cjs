...
class CzechTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const czechContent = fs.readFileSync(path.join(rootDir, 'locales/cs.json'), 'utf8');
      ...
      console.log(`🇨🇿 Loaded ${Object.keys(this.czechTranslations).length} Czech translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, czech, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateCzechVerb(english, czech)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Czech infinitive (-t/-ci) or imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInconsistentCzechCase(czech)) {
      issues.push({
        ...
        message: 'Check Czech case usage - ensure consistency with grammatical context',
        severity: 'medium'
      });
    }
    ...
    if (this.hasGenderAgreementIssues(czech)) {
      issues.push({
        ...
        message: 'Check Czech gender agreement (masculine animate/inanimate, feminine, neuter)',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkCzechVariableHandling(english, czech, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            czech,
  ...
  checkCzechVariableHandling(english, czech, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperCzechNumberAgreement(czech, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'Czech number-noun agreement may need attention (singular/plural/paucal forms)',
            severity: 'high'
          });
        }
      }
      ...
      if (varInfo.type === 'string' && this.needsCzechCaseAdjustment(czech, varName)) {
        issues.push({
          type: 'case_adjustment',
          variable: varName,
          message: 'Variable may need Czech case inflection in grammatical context',
          severity: 'medium'
        });
      }
  ...
  hasAppropriateCzechVerb(english, czech) {
    ...
    const czechVerbPatterns = [
      /t$/, /ci$/, /it$/, /et$/,    // infinitive endings
      /te$/, /ej$/, /i$/,          // imperative forms
    ];
    
    const actionWords = ['add', 'save', 'delete', 'create', 'update', 'send', 'cancel'];
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      return czechVerbPatterns.some(pattern => pattern.test(czech.toLowerCase()));
    }
  ...
  missingCzechHacek(english, czech) {
    ...
    return suspiciousPatterns.some(pattern => pattern.test(czech.toLowerCase()));
  }
  ...
  needsCzechCaseAdjustment(czech, varName) {
    ...
    return czech.includes(variablePattern) && 
  ...
  hasProblematicConsonantClusters(czech) {
    ...
    return difficultClusters.some(pattern => pattern.test(czech.toLowerCase()));
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('2. Review button text for Czech infinitive or imperative forms');
