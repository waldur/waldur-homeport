...
class AnalyzeTranslations {
  ...
  analyzeChanges(proposedTranslations) {
    ...
    for (const [key, proposedValue] of Object.entries(proposedTranslations)) {
      ...
      if (proposedValue === '[MISSING - ADD TRANSLATION]' || proposedValue === '[MISSING]') {
        if (!currentTranslations.hasOwnProperty(key)) {
          analysis.issues.push(`Missing translation for "${key}" - needs manual translation`);
        }
        continue;
      }
      ...
      if (!currentTranslations.hasOwnProperty(key)) {
        ...
        const issue = this.identifyIssue(key, currentValue, proposedValue, template[key]);
  ...
  identifyIssue(key, current, proposed, templateData) {
    ...
    if (templateData?.context?.primary_ui_type) {
      ...
      if (uiType.includes('error') && !current.includes('.') && current.length > 10) {
  ...
  checkLanguageSpecificIssues(current, proposed) {
    ...
    switch (this.language) {
      case 'ru':
        ...
        if (current.includes('перезаряжать') && proposed.includes('перезагрузить')) {
          issues.push('incorrect verb form (reload should be перезагрузить, not перезаряжать)');
        }
        break;
        
      case 'et':
        // Estonian-specific checks
        if (current.includes(' ') && !proposed.includes(' ') && proposed.length > current.length) {
    ...
    switch (command) {
      case 'analyze-file':
        ...
        if (!inputFile || !fs.existsSync(inputFile)) {
          console.error('Usage: node analyzeTranslations.cjs analyze-file <language> <input.json>');
          process.exit(1);
        }
        const proposedTranslations = JSON.parse(fs.readFileSync(inputFile, 'utf8'));
      ...
      case 'changes-only':
        ...
        if (!inputFile2 || !fs.existsSync(inputFile2)) {
          console.error('Usage: node analyzeTranslations.cjs changes-only <language> <input.json>');
          process.exit(1);
        }
        const proposedTranslations2 = JSON.parse(fs.readFileSync(inputFile2, 'utf8'));
      ...
      case 'validate-keys':
        ...
        if (keys.length === 0) {
          console.error('Usage: node analyzeTranslations.cjs validate-keys <language> <key1> <key2> ...');
          process.exit(1);
        }
      ...
      default:
        ...
        console.log('  analyze-file <language> <input.json>      - Analyze proposed translations');
        console.log('  changes-only <language> <input.json>      - Get only translations that need changes');
        console.log('  validate-keys <language> <key1> <key2>... - Validate translation keys');
        break;
    }
  } catch (error) {
