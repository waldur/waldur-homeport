...
class PersianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const persianContent = fs.readFileSync(path.join(rootDir, 'locales/fa.json'), 'utf8');
      ...
      console.log(`🇮🇷 Loaded ${Object.keys(this.persianTranslations).length} Persian translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, persian, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriatePersianVerb(english, persian)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using appropriate Persian verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.shouldPreferPersianOverArabic(english, persian)) {
      issues.push({
        ...
        message: 'Consider using Persian vocabulary instead of Arabic loanwords',
        severity: 'low'
      });
    }
    ...
    if (this.needsFormalRegisterMarkers(english, persian)) {
      issues.push({
        ...
        message: 'Consider using formal Persian register markers for professional context',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkPersianVariableHandling(english, persian, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            persian,
  ...
  checkPersianTitle(english, persian) {
    ...
    if (this.shouldUseNativePersianTerms(english, persian)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native Persian terminology instead of loanwords',
        severity: 'medium'
      });
    }
    ...
    if (this.needsFormalPersianLanguage(persian)) {
      issues.push({
        type: 'formality',
        message: 'Consider using formal Persian language appropriate for titles',
        severity: 'low'
      });
    }
  ...
  hasAppropriatePersianVerb(english, persian) {
    ...
    const persianActionVerbs = /(اضافه کنید|ذخیره کنید|حذف کنید|ایجاد کنید|به‌روزرسانی|ارسال|لغو)/;
  ...
  needsFormalRegisterMarkers(english, persian) {
    ...
    const contextNeedsFormal = english.includes('please') || english.includes('kindly');
  ...
  needsEzafeWithVariable(persian, varName) {
    ...
    if (persian.includes(numberVar)) {
      ...
      return /[آاهیة]$/.test(beforeVar.trim()) && /^[آاهیة]/.test(afterVar.trim());
    }
    return false;
  }
