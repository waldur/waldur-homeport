...
class SwedishTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const swedishContent = fs.readFileSync(path.join(rootDir, 'locales/sv.json'), 'utf8');
      ...
      console.log(`🇸🇪 Loaded ${Object.keys(this.swedishTranslations).length} Swedish translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, swedish, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateSwedishVerb(english, swedish)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Swedish imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasIncorrectSwedishDefiniteArticle(swedish)) {
      issues.push({
        ...
        message: 'Check Swedish definite article suffix usage (-en, -et, -na, -a)',
        severity: 'medium'
      });
    }
    ...
    if (this.shouldUseSwedishCompound(english, swedish)) {
      issues.push({
        type: 'compound_word',
        message: 'Consider Swedish compound word construction for better readability',
        severity: 'low'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkSwedishVariableHandling(english, swedish, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            swedish,
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperSwedishNumberAgreement(swedish, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'Swedish number-noun agreement may need attention (singular/plural forms)',
            severity: 'high'
          });
        }
      }
      ...
      if (this.needsSwedishGenderAgreement(swedish, varName)) {
        issues.push({
          ...
          variable: varName,
          message: 'Variable may need Swedish gender agreement consideration (en/ett)',
          severity: 'medium'
        });
      }
    }
  ...
  checkSwedishTitle(english, swedish) {
    ...
    if (this.shouldUseNativeSwedishTerms(english, swedish)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native Swedish terminology instead of anglicisms',
        severity: 'medium'
      });
    }
  ...
  hasAppropriateSwedishVerb(english, swedish) {
    ...
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      return swedishImperativePatterns.some(pattern => pattern.test(swedish.toLowerCase())) ||
             swedish.toLowerCase().match(/^(lägg till|spara|radera|skapa|uppdatera|skicka|avbryt)/);
    }
  ...
  shouldUseSwedishCompound(english, swedish) {
    ...
    return englishWords.length === 2 && swedishWords.length >= 2 && 
           !swedish.includes(' och ') && !swedish.includes(' eller '); // Not with conjunctions
  }
  ...
  needsSwedishGenderAgreement(swedish, varName) {
    ...
    return swedish.includes(numberVar) && swedish.match(/\s(den|det|denna|detta)\s/);
  }
  ...
  shouldUseNativeSwedishTerms(english, swedish) {
    ...
    return anglicisms.some(anglicism => swedish.toLowerCase().includes(anglicism));
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log('1. Focus on definite article suffix usage (-en, -et, -na, -a)');
