...
class SlovenianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const slovenianContent = fs.readFileSync(path.join(rootDir, 'locales/sl.json'), 'utf8');
      ...
      console.log(`🇸🇮 Loaded ${Object.keys(this.slovenianTranslations).length} Slovenian translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, slovenian, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateSlovenianVerb(english, slovenian)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Slovenian imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInconsistentSlovenianCase(slovenian)) {
      issues.push({
        ...
        message: 'Check Slovenian case usage (6 cases) - ensure consistency with context',
        severity: 'high'
      });
    }
    ...
    if (this.hasInconsistentSlovenianGender(slovenian)) {
      issues.push({
        ...
        message: 'Check Slovenian gender agreement (masculine, feminine, neuter)',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkSlovenianVariableHandling(english, slovenian, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            slovenian,
  ...
  checkSlovenianVariableHandling(english, slovenian, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperSlovenianNumberAgreement(slovenian, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'Slovenian number-noun agreement needs attention (1, 2, 3-4, 5+)',
            severity: 'high'
          });
        }
      ...
      if (varInfo.type === 'string' && this.needsSlovenianCaseAdjustment(slovenian, varName)) {
        issues.push({
      ...
      if (this.needsVerbAspectConsistency(slovenian, varName)) {
        issues.push({
          type: 'verb_aspect',
          variable: varName,
          message: 'Check Slovenian verb aspect (perfective/imperfective) consistency',
          severity: 'medium'
        });
      }
    }
  ...
  checkSlovenianTitle(english, slovenian) {
    ...
    if (this.shouldUseNativeSlovenianTerms(english, slovenian)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native Slovenian terminology instead of loanwords',
        severity: 'medium'
      });
    }
  ...
  hasAppropriateSlovenianVerb(english, slovenian) {
    ...
    const slovenianImperativePatterns = [
      /[ij]$/, /[ite]te$/, /[imo]mo$/ // Common imperative endings
    ];
    
    const actionWords = ['add', 'save', 'delete', 'create', 'update', 'send', 'cancel'];
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      return slovenianImperativePatterns.some(pattern => pattern.test(slovenian.toLowerCase()));
    }
  ...
  needsDualNumberForms(slovenian) {
    ...
    return slovenian.includes('2') || slovenian.includes('dva') || slovenian.includes('dve') ||
           slovenian.match(/\b(oba|obe)\b/);
  }

  hasInconsistentFormality(slovenian) {
    // Check for mixing formal and informal address
    return slovenian.includes('vi ') && slovenian.includes('ti ') ||
  ...
  hasMissingDiacriticals(slovenian) {
    ...
    return wordsNeedingDiacriticals.some(word => 
      slovenian.toLowerCase().includes(word) && 
      !slovenian.includes(word.replace('c', 'č').replace('s', 'š').replace('z', 'ž'))
    );
  }
  ...
  hasProperSlovenianNumberAgreement(slovenian, varName) {
    ...
    if (slovenian.includes(numberVar)) {
      ...
      return slovenian.includes('|') || // Indicates plural handling
             slovenian.match(/\{.*\}\s*\|\s*\{.*\}/); // Dual/plural forms
    }
    return true;
  }
  ...
  hasDualNumberHandling(slovenian, varName) {
    ...
    if (slovenian.includes(numberVar)) {
      ...
      return slovenian.includes('dva') || slovenian.includes('dve') ||
             slovenian.match(/\{.*2.*\}/);
    }
    return true;
  }
  ...
  needsSlovenianCaseAdjustment(slovenian, varName) {
    ...
    return slovenian.includes(numberVar) && 
  ...
  needsVerbAspectConsistency(slovenian, varName) {
    ...
    return slovenian.includes(`{${varName}}`) && 
           (slovenian.includes('bo') || slovenian.includes('je') || slovenian.includes('bi'));
  }
  ...
  shouldUseDiminutiveForms(slovenian) {
    ...
    return harshWords.some(word => slovenian.includes(word)) && 
  ...
  shouldUseNativeSlovenianTerms(english, slovenian) {
    ...
    return loanwords.some(loanword => slovenian.toLowerCase().includes(loanword));
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log('1. Focus on case system implementation (6 cases) and agreement');
