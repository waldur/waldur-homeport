...
class TranslationCleaner {
  ...
  cleanup(dryRun = false) {
    ...
    analyzer.allTsFiles = analyzer.getAllTSFiles(
      analyzer.constructor.CONFIG?.srcDir || path.join(__dirname, '../../src'),
    );

    console.log(
      `📁 Processing ${analyzer.allTsFiles.length} TypeScript files...`,
    );
    ...
    console.log(
      `✅ Found ${analyzer.usedTranslations.size} used translation keys\n`,
    );
    ...
    if (dryRun) {
      ...
      console.log(
        `   Would remove ${unusedTranslations.size} unused translation keys`,
      );
      return;
    }
    ...
    console.log(
      `   Template now contains ${Object.keys(cleanedTemplate).length} translation keys`,
    );
