...
class SpanishTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const spanishContent = fs.readFileSync(path.join(rootDir, 'locales/es.json'), 'utf8');
      ...
      console.log(`🇪🇸 Loaded ${Object.keys(this.spanishTranslations).length} Spanish translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, spanish, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateSpanishVerb(english, spanish)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Spanish imperative verb form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInconsistentSpanishFormality(spanish)) {
      issues.push({
        ...
        message: 'Check usted/tú consistency - use formal "usted" in professional contexts',
        severity: 'high'
      });
    }
    ...
    if (this.hasMissingAccents(spanish)) {
      issues.push({
        type: 'accent_marks',
        message: 'Check for missing or incorrect accent marks (á, é, í, ó, ú, ñ)',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkSpanishVariableHandling(english, spanish, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            spanish,
  ...
  checkSpanishVariableHandling(english, spanish, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperSpanishNumberAgreement(spanish, varName)) {
          issues.push({
            ...
            variable: varName,
            message: 'Spanish number-noun agreement may need attention (singular/plural)',
            severity: 'high'
          });
        }
      }
      ...
      if (varInfo.type === 'string' && this.needsSpanishGenderAgreement(spanish, varName)) {
        issues.push({
          type: 'gender_agreement',
          variable: varName,
          message: 'Variable may need Spanish gender agreement (masculine/feminine)',
          severity: 'high'
        });
      }
      ...
      if (this.needsSpanishSubjunctive(spanish, varName)) {
        issues.push({
          ...
          variable: varName,
          message: 'Consider using Spanish subjunctive mood for uncertainty/doubt',
          severity: 'medium'
        });
      }
    }
  ...
  checkSpanishTitle(english, spanish) {
    ...
    if (this.shouldUseNativeSpanishTerms(english, spanish)) {
      issues.push({
        type: 'terminology',
        message: 'Consider using native Spanish terminology instead of Anglicisms',
        severity: 'medium'
      });
    }
  ...
  hasAppropriateSpanishVerb(english, spanish) {
    ...
    const spanishImperativePatterns = [
      ...
      /e$/, /a$/,             // Imperative endings
      /ad$/, /ed$/, /id$/,    // Plural imperative endings
    ];
    ...
    if (actionWords.some(word => english.toLowerCase().includes(word))) {
      ...
      const commonActionVerbs = ['agregar', 'guardar', 'eliminar', 'crear', 'actualizar', 'enviar', 'cancelar'];
      return commonActionVerbs.some(verb => spanish.toLowerCase().includes(verb)) ||
             spanishImperativePatterns.some(pattern => pattern.test(spanish.toLowerCase()));
    }
  ...
  hasInconsistentSpanishFormality(spanish) {
    ...
    const hasFormal = formalPatterns.some(pattern => spanish.toLowerCase().includes(pattern));
    const hasInformal = informalPatterns.some(pattern => spanish.toLowerCase().includes(pattern));
  ...
  hasProperSpanishGenderAgreement(spanish) {
    ...
    return !mismatchPatterns.some(pattern => pattern.test(spanish.toLowerCase()));
  }
  ...
  needsReflexivePronouns(english, spanish) {
    ...
    const reflexiveVerbs = ['login', 'register', 'connect', 'disconnect', 'change'];
    const spanishReflexive = ['se', 'me', 'te', 'nos', 'os'];
    
    if (reflexiveVerbs.some(verb => english.toLowerCase().includes(verb))) {
      return !spanishReflexive.some(pronoun => spanish.toLowerCase().includes(pronoun));
    }
    return false;
  }
  ...
  hasProperSpanishNumberAgreement(spanish, varName) {
    ...
    if (spanish.includes(numberVar)) {
      ...
      return pluralContext.match(/\{[^}]+\}\s+\w+(s|es)\b/) !== null ||
             spanish.includes('uno') || spanish.includes('una'); // Singular forms
    }
    return true;
  }
  ...
  needsSpanishGenderAgreement(spanish, varName) {
    ...
    return spanish.includes(numberVar) && 
  ...
  needsSpanishSubjunctive(spanish, varName) {
    ...
    return subjunctiveMarkers.some(marker => spanish.includes(marker)) &&
  ...
  hasAppropriateSpanishFormality(spanish) {
    ...
    return formalMarkers.some(marker => spanish.toLowerCase().includes(marker));
  }
  ...
  isSpanishErrorTooAbrupt(spanish) {
    ...
    return abruptPatterns.some(pattern => 
      ...
      !spanish.includes('por favor') && !spanish.includes('disculpe')
    );
  }
  ...
  shouldUseSpanishSubjunctiveInError(spanish) {
    ...
    return spanish.toLowerCase().includes('error') && 
  ...
  shouldUseNativeSpanishTerms(english, spanish) {
    ...
    return anglicisms.some(anglicism => spanish.toLowerCase().includes(anglicism));
  }
