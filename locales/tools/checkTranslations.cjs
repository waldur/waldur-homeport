...
const CONFIG = {
  ...
  excludeFiles: ['.spec.ts', '.spec.tsx', '.test.ts', '.test.tsx', '.fixture.ts', '.fixture.tsx'],
...
class TranslationAnalyzer {
  ...
  getAllTSFiles(dirPath, arrayOfFiles = []) {
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        if (!CONFIG.excludeDirs.some(excludeDir => fullPath.includes(excludeDir))) {
  ...
  extractUsedTranslations(node, filePath) {
    ...
    ts.forEachChild(node, childNode => this.extractUsedTranslations(childNode, filePath));
  }
  ...
  findPotentialMissingTranslations(node, filePath) {
    if (ts.isStringLiteral(node)) {
      ...
      const looksLikeUIText = CONFIG.uiStringPatterns.some(pattern => pattern.test(text));
      ...
      if (looksLikeUIText || (hasSpaces && startsWithCapital)) {
        ...
        this.potentialMissing.get(text).files.add(`${relativeFilePath}:${line}`);
        this.potentialMissing.get(text).contexts.push(context);
      }
    }
    
    ts.forEachChild(node, childNode => this.findPotentialMissingTranslations(childNode, filePath));
  }
  ...
  loadLocales() {
    ...
    try {
      ...
      console.warn(`Warning: Could not read locales directory: ${error.message}`);
    }
  ...
  analyzeLocales(template, usedTranslations, filterLocale = null) {
    ...
    const locales = filterLocale ? { [filterLocale]: allLocales[filterLocale] } : allLocales;
    
    if (filterLocale && !locales[filterLocale]) {
      console.warn(`⚠️  Language '${filterLocale}' not found. Available languages: ${Object.keys(allLocales).join(', ')}`);
      return {};
    }
    ...
    for (const [locale, translations] of Object.entries(locales)) {
      ...
      const deprecatedInLocale = [...localeKeys].filter(key => !usedTranslations.has(key));
      
      // Find template keys not in locale (may be unused but in template)
      const templateNotInLocale = allTemplateKeys.filter(key => !localeKeys.has(key));
      ...
      analysis[locale] = {
        ...
        coverage: usedKeys.length > 0 ? ((usedKeys.length - missingInLocale.length) / usedKeys.length * 100) : 100
      };
    }
  ...
  analyze(filterLocale = null) {
    ...
    console.log(`✅ Found ${this.usedTranslations.size} used translation keys\n`);
    
    // Find unused translations
    const unusedTranslations = new Set([...allTemplateKeys].filter(key => !this.usedTranslations.has(key)));
    
    // Analyze locales (with optional filter)
    const localeAnalysis = this.analyzeLocales(template, this.usedTranslations, filterLocale);
    ...
    this.generateSummary(allTemplateKeys.size, this.usedTranslations.size, unusedTranslations.size, localeAnalysis);
  }
  ...
  generateMissingReport() {
    ...
    console.log(`🚨 POTENTIAL MISSING TRANSLATIONS (${this.potentialMissing.size} found):`);
    console.log('=' .repeat(50));
    
    const sortedMissing = [...this.potentialMissing.entries()].sort(([a], [b]) => a.localeCompare(b));
  ...
  generateLocaleReport(localeAnalysis) {
    ...
    console.log('┌─────────┬─────────┬─────────┬─────────┬─────────┬─────────┐');
    console.log('│ Locale  │  Total  │ Missing │ Deprecated│  Empty  │Coverage │');
    console.log('├─────────┼─────────┼─────────┼─────────┼─────────┼─────────┤');
    
    for (const [locale, analysis] of Object.entries(localeAnalysis)) {
      const coverage = `${analysis.coverage.toFixed(1)}%`;
      console.log(`│ ${locale.padEnd(7)} │ ${String(analysis.totalKeys).padStart(7)} │ ${String(analysis.missingKeys.length).padStart(7)} │ ${String(analysis.deprecatedKeys.length).padStart(9)} │ ${String(analysis.emptyTranslations.length).padStart(7)} │ ${coverage.padStart(7)} │`);
    }
    console.log('└─────────┴─────────┴─────────┴─────────┴─────────┴─────────┘\n');

    // Detailed analysis for languages with issues
    const problemLocales = Object.entries(localeAnalysis).filter(([_, analysis]) => 
      analysis.missingKeys.length > 0 || analysis.deprecatedKeys.length > 10 || analysis.emptyTranslations.length > 0
    );
    ...
    if (problemLocales.length > 0) {
      ...
      for (const [locale, analysis] of problemLocales) {
        ...
        if (analysis.missingKeys.length > 0) {
          console.log(`  ❌ Missing ${analysis.missingKeys.length} required translations`);
          if (analysis.missingKeys.length <= 10) {
            analysis.missingKeys.forEach(key => console.log(`     "${key}"`));
          } else {
            analysis.missingKeys.slice(0, 5).forEach(key => console.log(`     "${key}"`));
            console.log(`     ... and ${analysis.missingKeys.length - 5} more`);
          }
        }
        
        if (analysis.deprecatedKeys.length > 10) {
          console.log(`  🗑️  ${analysis.deprecatedKeys.length} deprecated translations (consider cleanup)`);
        }
        
        if (analysis.emptyTranslations.length > 0) {
          console.log(`  📝 ${analysis.emptyTranslations.length} empty translations need content`);
          if (analysis.emptyTranslations.length <= 5) {
            analysis.emptyTranslations.forEach(key => console.log(`     "${key}"`));
          } else {
            analysis.emptyTranslations.slice(0, 3).forEach(key => console.log(`     "${key}"`));
            console.log(`     ... and ${analysis.emptyTranslations.length - 3} more`);
          }
        }
      }
    }
  ...
  generateSummary(totalKeys, usedKeys, unusedKeys, localeAnalysis) {
    ...
    console.log(`Potential missing translations: ${this.potentialMissing.size}`);
    ...
    if (localeAnalysis && Object.keys(localeAnalysis).length > 0) {
      ...
      const totalMissing = locales.reduce((sum, l) => sum + l.missingKeys.length, 0);
      const totalDeprecated = locales.reduce((sum, l) => sum + l.deprecatedKeys.length, 0);
      const totalEmpty = locales.reduce((sum, l) => sum + l.emptyTranslations.length, 0);
...
Examples:
  ...
  if (options.dryRun || options.summary) {
    ...
    analyzer.generateUnusedReport = function(unusedTranslations) {
      if (unusedTranslations.size > 0) {
        console.log(`⚠️  Found ${unusedTranslations.size} unused translations in template.json\n`);
      } else {
    ...
    analyzer.generateMissingReport = function() {
      if (this.potentialMissing.size > 0) {
        console.log(`🚨 Found ${this.potentialMissing.size} potential missing translations in codebase\n`);
      } else {
    ...
    analyzer.generateLocaleReport = function(localeAnalysis) {
      ...
      for (const [locale, analysis] of Object.entries(localeAnalysis)) {
        const status = analysis.coverage >= 95 ? '✅' : 
                      analysis.coverage >= 80 ? '⚠️ ' : '❌';
        console.log(`${status} ${locale}: ${analysis.coverage.toFixed(1)}% coverage (${analysis.missingKeys.length} missing, ${analysis.deprecatedKeys.length} deprecated, ${analysis.emptyTranslations.length} empty)`);
      }
      console.log('');
    };
  }
  ...
  if (options.localesOnly) {
    ...
    const modeFilter = options.language ? ` for language '${options.language}'` : '';
    ...
    const localeAnalysis = analyzer.analyzeLocales(template, analyzer.usedTranslations, options.language);
    ...
    analyzer.generateSummary(allTemplateKeys.size, analyzer.usedTranslations.size, unusedTranslations.size);
