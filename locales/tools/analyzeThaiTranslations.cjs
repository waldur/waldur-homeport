...
class ThaiTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const thaiContent = fs.readFileSync(path.join(rootDir, 'locales/th.json'), 'utf8');
      ...
      console.log(`🇹🇭 Loaded ${Object.keys(this.thaiTranslations).length} Thai translations`);
    } catch (error) {
  ...
  checkButtonTextQuality(english, thai, context) {
    ...
    if (context.primary_ui_type === 'submit_button' || context.primary_ui_type === 'action_button') {
      if (!this.hasAppropriateThaiVerb(english, thai)) {
        issues.push({
          type: 'verb_form',
          message: 'Consider using Thai imperative or polite request form for action buttons',
          severity: 'low'
        });
      }
    }
    ...
    if (this.hasInappropriateThaiSpacing(thai)) {
      issues.push({
        type: 'word_spacing',
        message: 'Check Thai word spacing - Thai typically doesn\'t use spaces between words',
        severity: 'low'
      });
    }
    ...
    if (this.needsFormalRegisterMarkers(english, thai)) {
      issues.push({
        ...
        message: 'Consider using formal Thai register markers for professional context',
        severity: 'medium'
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(this.enhancedTemplate)) {
      ...
      if (context && context.variables) {
        const issues = this.checkThaiVariableHandling(english, thai, context.variables);
        if (issues.length > 0) {
          variableIssues.push({
            english,
            thai,
  ...
  needsThaiPolitenessParticles(english, thai, context) {
    const needsPoliteness = context.primary_ui_type === 'submit_button' || 
  ...
  hasInappropriateThaiSpacing(thai) {
    ...
    return wordCount > 3 && (thai.length - nonSpaceLength) / nonSpaceLength > 0.3;
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('5. Consider native Thai terms over loanwords where appropriate');
  }
}
