  ...
  searchPattern(pattern, searchIn = 'both', limit = 50) {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(translations)) {
        ...
        const keyMatch =
          searchIn === 'keys' || searchIn === 'both' ? regex.test(key) : false;
        const valueMatch =
  ...
  try {
    switch (command) {
      case 'search-keys':
        ...
        if (keys.length === 0) {
          console.error(
            'Usage: node searchTranslations.cjs search-keys <language> <key1> <key2> ...',
          );
          process.exit(1);
        }
      ...
      case 'search-pattern':
        ...
        if (!pattern) {
          console.error(
            'Usage: node searchTranslations.cjs search-pattern <language> <pattern> [keys|values|both] [limit]',
          );
          process.exit(1);
        }
      ...
      case 'check-exist':
        ...
        if (checkKeys.length === 0) {
          console.error(
            'Usage: node searchTranslations.cjs check-exist <language> <key1> <key2> ...',
          );
          process.exit(1);
        }
      ...
      default:
        ...
        console.log(
          '  search-keys <language> <key1> <key2> ...     - Search for specific keys',
        );
        console.log(
          '  search-pattern <language> <pattern> [where]  - Search with regex pattern',
        );
        console.log(
          '  check-exist <language> <key1> <key2> ...     - Check if keys exist',
        );
        console.log(
          '  stats <language>                             - Get locale file statistics',
        );
        break;
    }
  } catch (error) {
