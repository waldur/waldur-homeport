...
class DanishTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const danishContent = fs.readFileSync(
        path.join(rootDir, 'locales/da.json'),
        'utf8',
      );
      ...
      console.log(
        `🇩🇰 Loaded ${Object.keys(this.danishTranslations).length} Danish translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, danish, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateDanishVerb(english, danish)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Danish imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasIncorrectDanishDefiniteArticle(danish)) {
      issues.push({
        ...
        message:
          'Check Danish definite article suffix usage (-en, -et, -ene, -ne)',
        severity: 'medium',
      });
    }
    ...
    if (this.shouldUseDanishCompound(english, danish)) {
      issues.push({
        type: 'compound_word',
        message:
          'Consider Danish compound word construction for better readability',
        severity: 'low',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkDanishVariableHandling(
          english,
          danish,
          context.variables,
        );
      ...
      if (varInfo.type === 'number') {
        if (!this.hasProperDanishNumberAgreement(danish, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'Danish number-noun agreement may need attention (singular/plural forms)',
            severity: 'high',
          });
        }
      }
  ...
  checkDanishTitle(english, danish) {
    ...
    if (this.shouldUseNativeDanishTerms(english, danish)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Danish terminology instead of anglicisms',
        severity: 'medium',
      });
    }
  ...
  hasAppropriateDanishVerb(english, danish) {
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      return (
        danishImperativePatterns.some((pattern) =>
          pattern.test(danish.toLowerCase()),
        ) ||
        danish
          .toLowerCase()
          .match(/^(tilføj|gem|slet|opret|opdater|send|annuller)/)
      );
    }
  ...
  shouldUseDanishCompound(english, danish) {
    ...
    return (
      ...
      danishWords.length >= 2 &&
      !danish.includes(' og ') &&
      !danish.includes(' eller ')
  ...
  needsDanishGenderAgreement(danish, varName) {
    ...
    return (
      danish.includes(numberVar) && danish.match(/\s(den|det|denne|dette)\s/)
    );
  }
  ...
  shouldUseNativeDanishTerms(english, danish) {
    ...
    return anglicisms.some((anglicism) =>
      danish.toLowerCase().includes(anglicism),
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log(
      '1. Focus on definite article suffix usage (-en, -et, -ene, -ne)',
    );
