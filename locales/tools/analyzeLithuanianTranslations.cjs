...
class LithuanianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const lithuanianContent = fs.readFileSync(
        path.join(rootDir, 'locales/lt.json'),
        'utf8',
      );
      ...
      console.log(
        `🇱🇹 Loaded ${Object.keys(this.lithuanianTranslations).length} Lithuanian translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, lithuanian, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateLithuanianVerb(english, lithuanian)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Lithuanian infinitive (-ti) or imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasInconsistentLithuanianCase(lithuanian)) {
      issues.push({
        ...
        message:
          'Check Lithuanian case usage - ensure consistency with grammatical context',
        severity: 'medium',
      });
    }
    ...
    if (this.hasGenderAgreementIssues(lithuanian)) {
      issues.push({
        ...
        message:
          'Check Lithuanian gender agreement (masculine/feminine) in adjectives and participles',
        severity: 'medium',
      });
    }
    ...
    if (this.missingLithuanianDiacriticals(english, lithuanian)) {
      issues.push({
        type: 'diacriticals',
        message:
          'Check Lithuanian diacritical marks (ą, č, ę, ė, į, š, ų, ū, ž)',
        severity: 'high',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkLithuanianVariableHandling(
          english,
          lithuanian,
          context.variables,
        );
  ...
  checkLithuanianVariableHandling(english, lithuanian, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperLithuanianNumberAgreement(lithuanian, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'Lithuanian number-noun agreement may need attention (singular/dual/plural forms)',
            severity: 'high',
          });
        }
      }
      ...
      if (
        varInfo.type === 'string' &&
        this.needsLithuanianCaseAdjustment(lithuanian, varName)
      ) {
        issues.push({
          type: 'case_adjustment',
          variable: varName,
          message:
            'Variable may need Lithuanian case inflection in grammatical context',
          severity: 'medium',
        });
      }
  ...
  checkLithuanianTitle(english, lithuanian) {
    ...
    if (this.hasInappropriateLithuanianCapitalization(lithuanian)) {
      issues.push({
        type: 'capitalization',
        message:
          'Lithuanian titles typically use sentence case, not title case',
        severity: 'low',
      });
    }
    ...
    if (this.shouldUseNativeLithuanianTerms(english, lithuanian)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Lithuanian terminology instead of loanwords',
        severity: 'medium',
      });
    }
    ...
    if (this.shouldUseLithuanianDiminutive(english, lithuanian)) {
      issues.push({
        type: 'diminutive',
        message:
          'Consider Lithuanian diminutive forms for more natural expression',
        severity: 'low',
      });
    }
  ...
  hasAppropriateLithuanianVerb(english, lithuanian) {
    ...
    const lithuanianInfinitivePatterns = [
      /ti$/,
      /dyti$/,
      /ėti$/, // infinitive endings
      /k$/,
      /kie$/, // imperative forms
    ];
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      return lithuanianInfinitivePatterns.some((pattern) =>
        pattern.test(lithuanian.toLowerCase()),
      );
    }
  ...
  needsLithuanianCaseAdjustment(lithuanian, varName) {
    ...
    return (
      lithuanian.includes(variablePattern) &&
  ...
  isLithuanianErrorTooAbrupt(lithuanian) {
    ...
    return abruptPatterns.some((pattern) => {
      return (
        lithuanian.toLowerCase().startsWith(pattern) &&
        !lithuanian.includes('prašome')
      );
    });
  }
  ...
  shouldUseNativeLithuanianTerms(english, lithuanian) {
    ...
    return loanwords.some((loanword) =>
      lithuanian.toLowerCase().includes(loanword),
    );
  }
  ...
  shouldUseLithuanianDiminutive(english, lithuanian) {
    ...
    return (
      english.toLowerCase().includes('small') &&
      !lithuanian.match(/[ėė]lis|[uū]kas|ytė$/)
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log(
      '2. Review button text for Lithuanian infinitive or imperative forms',
    );
    console.log('3. Ensure proper diacritical marks usage');
    console.log('4. Check dual number forms for quantities of 2, 12, 22, etc.');
    console.log(
      '6. Standardize title capitalization to Lithuanian conventions',
    );
  }
}
