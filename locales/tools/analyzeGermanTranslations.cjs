...
class GermanTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const germanContent = fs.readFileSync(
        path.join(rootDir, 'locales/de.json'),
        'utf8',
      );
      ...
      console.log(
        `🇩🇪 Loaded ${Object.keys(this.germanTranslations).length} German translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, german, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateGermanVerb(english, german)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using German imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasInconsistentGermanFormality(german)) {
      issues.push({
        ...
        message:
          'Check Sie/du consistency - use formal "Sie" in professional contexts',
        severity: 'high',
      });
    }
    ...
    if (this.shouldUseGermanCompound(english, german)) {
      issues.push({
        type: 'compound_word',
        message:
          'Consider German compound word construction for better readability',
        severity: 'low',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkGermanVariableHandling(
          english,
          german,
          context.variables,
        );
  ...
  checkGermanVariableHandling(english, german, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperGermanNumberAgreement(german, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'German number-noun agreement may need attention (singular/plural forms)',
            severity: 'high',
          });
        }
      }
      ...
      if (
        varInfo.type === 'string' &&
        this.needsGermanCaseAdjustment(german, varName)
      ) {
        issues.push({
  ...
  hasAppropriateGermanVerb(english, german) {
    ...
    const germanImperativePatterns = [
      /en$/,
      /e$/,
      /t$/, // Common imperative endings
      /ieren$/,
      /ern$/, // Common verb endings
    ];
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      ...
      return (
        german.includes('hinzufügen') ||
        german.includes('speichern') ||
        german.includes('löschen') ||
        german.includes('erstellen') ||
        germanImperativePatterns.some((pattern) =>
          pattern.test(german.toLowerCase()),
        )
      );
    }
  ...
  hasProperGermanCapitalization(german) {
    ...
    for (const word of words) {
      ...
      if (
        commonNouns.includes(lowerWord) &&
  ...
  isGermanErrorTooAbrupt(german) {
    ...
    return abruptPatterns.some(
      (pattern) =>
        ...
        !german.includes('bitte') &&
        !german.includes('leider'),
    );
  }
