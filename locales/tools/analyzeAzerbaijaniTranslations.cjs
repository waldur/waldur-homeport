...
class AzerbaijaniTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const azerbaijaniContent = fs.readFileSync(
        path.join(rootDir, 'locales/az.json'),
        'utf8',
      );
      ...
      console.log(
        `🇦🇿 Loaded ${Object.keys(this.azerbaijaniTranslations).length} Azerbaijani translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, azerbaijani, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateAzerbaijaniVerb(english, azerbaijani)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Azerbaijani imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.violatesAzerbaijaniVowelHarmony(azerbaijani)) {
      issues.push({
        type: 'vowel_harmony',
        message:
          'Check Azerbaijani vowel harmony (front/back vowel consistency)',
        severity: 'high',
      });
    }
    ...
    if (this.shouldPreferNativeAzerbaijaniTerms(english, azerbaijani)) {
      issues.push({
        ...
        message:
          'Consider using native Azerbaijani terms instead of Turkish loanwords',
        severity: 'low',
      });
    }
    ...
    if (this.hasMissingDiacriticals(azerbaijani)) {
      issues.push({
        type: 'diacriticals',
        message:
          'Check proper use of Azerbaijani diacriticals (ə, ö, ü, ğ, ı, ş, ç)',
        severity: 'medium',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkAzerbaijaniVariableHandling(
          english,
          azerbaijani,
          context.variables,
        );
  ...
  checkAzerbaijaniTitle(english, azerbaijani) {
    ...
    if (this.hasInappropriateAzerbaijaniCapitalization(azerbaijani)) {
      issues.push({
        type: 'capitalization',
        message:
          'Azerbaijani titles typically use sentence case, not title case',
        severity: 'low',
      });
    }
    ...
    if (this.shouldUseNativeAzerbaijaniTerms(english, azerbaijani)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Azerbaijani terminology instead of loanwords',
        severity: 'medium',
      });
    }
  ...
  hasAppropriateAzerbaijaniVerb(english, azerbaijani) {
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      return (
        azerbaijaniImperativePatterns.some((pattern) =>
          pattern.test(azerbaijani.toLowerCase()),
        ) ||
        azerbaijani
          .toLowerCase()
          .match(/^(əlavə et|saxla|sil|yarat|yenilə|göndər|ləğv et)/)
      );
    }
  ...
  hasMissingDiacriticals(azerbaijani) {
    ...
    const mightNeedDiacriticals =
      azerbaijani.length > 10 && /[aeiou]{2,}/.test(azerbaijani);
  ...
  needsAzerbaijaniCaseAdjustment(azerbaijani, varName) {
    ...
    return (
      azerbaijani.includes(numberVar) &&
  ...
  shouldUseNativeAzerbaijaniTerms(english, azerbaijani) {
    ...
    return loanwords.some((loanword) =>
      azerbaijani.toLowerCase().includes(loanword),
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log('\n🎯 PRIORITY ACTIONS:');
    console.log(
      '1. Focus on vowel harmony compliance (critical for Azerbaijani)',
    );
