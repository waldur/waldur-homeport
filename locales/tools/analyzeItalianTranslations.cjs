...
class ItalianTranslationAnalyzer {
  ...
  loadData() {
    try {
      ...
      const italianContent = fs.readFileSync(
        path.join(rootDir, 'locales/it.json'),
        'utf8',
      );
      ...
      console.log(
        `🇮🇹 Loaded ${Object.keys(this.italianTranslations).length} Italian translations`,
      );
    } catch (error) {
  ...
  checkButtonTextQuality(english, italian, context) {
    ...
    if (
      ...
      if (!this.hasAppropriateItalianVerb(english, italian)) {
        issues.push({
          type: 'verb_form',
          message:
            'Consider using Italian imperative verb form for action buttons',
          severity: 'low',
        });
      }
    }
    ...
    if (this.hasInconsistentItalianFormality(italian)) {
      issues.push({
        ...
        message:
          'Check Lei/tu consistency - use formal "Lei" in professional contexts',
        severity: 'high',
      });
    }
    ...
    if (this.hasMissingItalianAccents(italian)) {
      issues.push({
        type: 'accent_marks',
        message:
          'Check for missing accent marks on final vowels (à, è, ì, ò, ù)',
        severity: 'medium',
      });
    }
  ...
  analyzeVariableHandling() {
    ...
    for (const [english, templateData] of Object.entries(
      ...
      if (context && context.variables) {
        const issues = this.checkItalianVariableHandling(
          english,
          italian,
          context.variables,
        );
  ...
  checkItalianVariableHandling(english, italian, variables) {
    ...
    for (const [varName, varInfo] of Object.entries(variables)) {
      if (varInfo.type === 'number') {
        if (!this.hasProperItalianNumberAgreement(italian, varName)) {
          issues.push({
            ...
            variable: varName,
            message:
              'Italian number-noun agreement may need attention (singular/plural)',
            severity: 'high',
          });
        }
      }
      ...
      if (
        varInfo.type === 'string' &&
        this.needsItalianGenderAgreement(italian, varName)
      ) {
        issues.push({
          type: 'gender_agreement',
          variable: varName,
          message:
            'Variable may need Italian gender agreement (masculine/feminine)',
          severity: 'high',
        });
      }
      ...
      if (this.needsItalianVerbConjugation(italian, varName)) {
        issues.push({
          ...
          variable: varName,
          message:
            'Consider Italian verb conjugation for person/number agreement',
          severity: 'medium',
        });
      }
    }
  ...
  checkItalianTitle(english, italian) {
    ...
    if (this.shouldUseNativeItalianTerms(english, italian)) {
      issues.push({
        type: 'terminology',
        message:
          'Consider using native Italian terminology instead of Anglicisms',
        severity: 'medium',
      });
    }
  ...
  hasAppropriateItalianVerb(english, italian) {
    ...
    const italianImperativePatterns = [
      /are$/,
      /ere$/,
      /ire$/, // Infinitive endings
      /a$/,
      /i$/, // Imperative endings
      /ate$/,
      /ete$/,
      /ite$/, // Plural imperative endings
    ];
    ...
    if (actionWords.some((word) => english.toLowerCase().includes(word))) {
      ...
      return (
        commonActionVerbs.some((verb) =>
          italian.toLowerCase().includes(verb),
        ) ||
        italianImperativePatterns.some((pattern) =>
          pattern.test(italian.toLowerCase()),
        )
      );
    }
  ...
  hasMissingDoubleConsonants(italian) {
    ...
    for (const [correct, wrongPattern] of Object.entries(
      doubleConsonantWords,
    )) {
      if (
        wrongPattern.test(italian.toLowerCase()) &&
        !italian.toLowerCase().includes(correct)
      ) {
        return true;
      }
    }
  ...
  hasProperItalianNumberAgreement(italian, varName) {
    ...
    if (italian.includes(numberVar)) {
      ...
      return (
        pluralContext.match(/\{[^}]+\}\s+\w+(i|e)\b/) !== null ||
        italian.includes('uno') ||
        italian.includes('una')
      ); // Singular forms
    }
    return true;
  }
  ...
  needsItalianSubjunctive(italian) {
    ...
    return (
      subjunctiveMarkers.some((marker) => italian.includes(marker)) &&
      italian.includes('possibile')
    ); // Common context for subjunctive
  }
  ...
  shouldUseNativeItalianTerms(english, italian) {
    ...
    return anglicisms.some((anglicism) =>
      italian.toLowerCase().includes(anglicism),
    );
  }
  ...
  generateReport(buttonIssues, variableIssues, titleIssues) {
    ...
    console.log(
      '5. Review verb conjugations for proper person/number agreement',
    );
  }
}
