...
var Extractor = (function() {
  function Extractor(options) {
    ...
    this.strings = {};
    this.attrRegex = mkAttrRegex(
      this.options.startDelim,
      this.options.endDelim,
  ...
  Extractor.prototype.addString = function(
    reference,
    string,
    plural,
    extractedComment,
    ...
    if (extractedComment) {
      ...
      if (commentIndex < 0) {
        ...
        item.extractedComments.splice(
          Math.abs(commentIndex + 1),
          0,
          extractedComment,
        );
      }
  ...
  Extractor.prototype.extractJs = function(filename, src, lineNumber) {
      ...
      return (
        ...
        (self.options.markerNames.indexOf(node.callee.name) > -1 ||
          (node.callee.property &&
            self.options.markerNames.indexOf(node.callee.property.name) >
              -1)) &&
        node.arguments !== null &&
    ...
    walkJs(syntax, function(node, parentComment) {
      ...
      if (isGettext(node) || isGetString(node)) {
        ...
        var line =
          reference.location && reference.location.start.line
            ? reference.location.start.line - 1
            : 0;
      ...
      if (str || singular) {
        var leadingComments =
          ...
          (parentComment ? parentComment.leadingComments : []);
        ...
        if (str) {
          self.addString(
            ...
            str,
            plural,
            comments2String(extractedComments),
            context,
          );
        } else if (singular) {
          self.addString(
    ...
    var extractHtml = function(src, lineNumber) {
      ...
      $('*').each(function(index, n) {
        ...
        if (node.is(self.options.attribute)) {
          self.addString(
            reference(n.startIndex),
            str,
            extracted[self.options.attribute].plural,
            extracted[self.options.attribute].extractedComment,
            extracted[self.options.attribute].context,
          );
          return;
        }
  ...
  Extractor.prototype.isSupportedByStrategy = function(strategy, extension) {
    return (
      extension in this.options.extensions &&
      this.options.extensions[extension] === strategy
    );
  };
